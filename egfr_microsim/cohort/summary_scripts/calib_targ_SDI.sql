CREATE OR REPLACE TABLE {destination_dataset}.calib_targ_SDI AS (

    WITH filled_with_zeros AS (
      SELECT 
        person_id,
        g_stage,
        IFNULL(SDI_q3, 0) AS SDI_q3,
        CASE 
            WHEN (age < 30) THEN 20
    ...
    full_table AS (
      SELECT
        ...
        GROUP BY age_group, g_stage, SDI_q3
      ),
    ...
    USING(age_group, SDI_q3)
    ORDER BY age_group, SDI_q3
